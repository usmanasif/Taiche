		...
		if ( ( $pagenow == 'users.php' ) && empty( $_GET['page'] ) ) {
			if ( isset( $_REQUEST['group'] ) ) {
				...
				if ( Groups_Group::read( $group_id ) ) {
					...
					$user_query->query_where .= "join wp_groups_user_group 
on wp_groups_user_group.user_id = wp_users.id
WHERE wp_groups_user_group.group_id = $group_id";
	...
	public static function views_users( $views ) {
		...
		if ( ( $pagenow == 'users.php' ) && empty( $_GET['page'] ) ) {
			...
			$g_id = $wpdb->get_results( "SELECT group_id FROM $user_group_table WHERE user_id = $user_idd" );
			foreach( $g_id as $g ) {
			...
			$groups = $wpdb->get_results( "SELECT * FROM $group_table $where ORDER BY name" );
			foreach( $groups as $group ) {
				...
				$user_count = $wpdb->get_var( $wpdb->prepare(
					"SELECT count(wp_users.ID) FROM wp_users 
join wp_groups_user_group 
on wp_groups_user_group.user_id = wp_users.id
WHERE wp_groups_user_group.group_id = %d",
