		...
		case "order_actions" :
			...
			?><p>
				<?php
					...
					if ( in_array( $the_order->status, array(  'completed' ) ) )
						$actions['certified'] = array(
							'url' 		=> wp_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce-mark-order-certified&order_id=' . $post->ID ), 'woocommerce-mark-order-certified' ),
							'name' 		=> __( 'Certified', 'woocommerce' ),
							'action' 	=> "complete"
						);
					...
					foreach ( $actions as $action ) {
						if($action['name'] == 'Certified')
							{
							if(get_post_meta( $the_order->id, 'certify', 'true' ))								
								{printf('<p style="color: green;" >Certified</p>');
								continue;
									}	
							$checkin = ' Certify ';
							}
						...
						printf( '<a class="button tips" href="%s" data-tip="%s">'.$checkin.'<img src="%s" alt="%s" width="14" /></a>', esc_url( $action['url'] ), esc_attr( $action['name'] ), esc_attr( $image ), esc_attr( $action['name'] ) );
					}
