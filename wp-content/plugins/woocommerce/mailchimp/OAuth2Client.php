...
abstract class OAuth2Client {
  ...
  // So they're just stubs. Override the ones you need.
    ...
    public function getSession() {
        if ($this->debug)
            print('getSession:' . print_r($this->getVariable('_session'), true));
    ...
    private function getAccessTokenFromAuthorizationCode($code) {
        if ($this->debug)
            print('access_token_uri: ' . $this->getVariable('access_token_uri'));
    ...
    protected function makeRequest($path, $method = 'GET', $params = array(), $ch = NULL) {
        ...
        if ($this->debug)
            print('path = ' . $path);
        if ($this->debug)
            print('curl_opts = ' . print_r($opts, true));
    ...
    protected function validateSessionObject($session) {
        ...
        if (is_array($session) && isset($session['access_token']) && isset($session['sig'])) {
            ...
            $expected_sig = self::generateSignature(
                            $session_without_sig, $this->getVariable('client_secret')
            );
            if ($session['sig'] != $expected_sig) {
                if ($this->debug)
                    print("{$session['sig']} != {$expected_sig}");
