...
function woocommerce_mark_order_certified() {
...
$queryy = "
    SELECT `wp_users`.`ID`, `wp_users`.`user_email`   FROM wp_users where user_email IN (SELECT `wp_postmeta`.`meta_value` FROM wp_postmeta where `wp_postmeta`.post_id = $order_id  and `wp_postmeta`.`meta_key` = '_billing_email' )

    		   ";

	$user_id = $wpdb->get_results($queryy, OBJECT);
...
$queryy = "
   SELECT meta_value as certs FROM `wp_usermeta` WHERE `user_id` IN (SELECT `wp_users`.`ID`  FROM wp_users where user_email IN (SELECT `wp_postmeta`.`meta_value` FROM wp_postmeta where `wp_postmeta`.post_id = $order_id   and `wp_postmeta`.`meta_key` = '_billing_email' )
 )
and meta_key = 'certification'

    		   ";

	 $cur_user_certs = $wpdb->get_results($queryy, OBJECT);




$queryy = "
   SELECT DATE_ADD(CURDATE(),INTERVAL 27 MONTH) as date, meta_value, post_id FROM `wp_postmeta` where `post_id` IN (SELECT `wp_posts`.`ID` FROM `wp_posts`
					INNER JOIN `wp_postmeta`
					ON `wp_posts`.ID = `wp_postmeta`.`meta_value`
					INNER JOIN `wp_woocommerce_order_itemmeta`
					ON  `wp_woocommerce_order_itemmeta`.`meta_value` = `wp_postmeta`.post_id
					...
					WHERE `wp_posts`.`post_type` = 'tribe_events' and `wp_woocommerce_order_items`.`order_id` = $order_id     and `wp_postmeta`.meta_key = '_tribe_wooticket_for_event') and meta_key = '_EventHideFromUpcoming' ";


	$new_usr_certs = $wpdb->get_results($queryy, OBJECT);
$certi_meta = ",".$new_usr_certs[0]->meta_value;
$name = explode(",", $certi_meta);
$event_start_date = get_post_meta($new_usr_certs[0]->post_id,'_EventStartDate', true);
$date=date_create($event_start_date);
$temp_date = $date;
date_add($date,date_interval_create_from_date_string("27 months"));
date_add($temp_date,date_interval_create_from_date_string("24 months"));
$temp_date = date_format($temp_date,"Y-m-d");
$date = date_format($date,"Y-m-d");
$event_id = $new_usr_certs[0]->post_id;



$wpdb->query( $wpdb->prepare(
    "INSERT INTO wp_event_attendees (event_id, user_id) VALUES ($event_id, $us_id )"
   
));  		
foreach ($name as $cert_name )
{
	if ($cert_name == "")
	continue;	
	$queryy = "
   	SELECT `ID` FROM `wp_posts` WHERE `post_title` = '$cert_name' and post_type = 'certificate_template' and post_status = 'private'
    		   ";

	$cert_id = $wpdb->get_results($queryy, OBJECT);
	
	$queryy = "
   	SELECT max(number) as number FROM `wp_D`
    		   ";

	$max_number = $wpdb->get_results($queryy, OBJECT);	
	$cert_number = $max_number[0]->number+1;
	$num = sprintf('%06u', $cert_number);
	$wpdb->query( $wpdb->prepare(
    "INSERT INTO wp_Instructors_Certification (instructor_id , certification_id, certification_expiry_date, name, certificate_number, event_id) VALUES ( $us_id , {$cert_id[0]->ID} , '$date', '$cert_name', '$num', {$new_usr_certs[0]->post_id}  )"
   
));

$wpdb->query( $wpdb->prepare(
    "INSERT INTO `wp_D`(`number`) VALUES ($cert_number)"
   
));
// certification letters mandrill --- START
	$usr_frstname = get_user_meta($us_id,'first_name',true);
$usr_lstname = get_user_meta($us_id,'last_name',true);
$fullname = $usr_frstname." ".$usr_lstname;
$venue_id = get_post_meta($new_usr_certs[0]->post_id,'_EventVenueID', true);
$venue_location = get_post_meta($venue_id,'_VenueVenue', true);
$venue_country = get_post_meta($venue_id,'_VenueCountry', true);
$venue = $venue_location.",".$venue_country;
$event_obj = get_post($new_usr_certs[0]->post_id);
$mstr_frstname = get_user_meta($event_obj->post_author,'first_name',true);
$mstr_lstname = get_user_meta($event_obj->post_author,'last_name',true);
$fullname_mstr = $mstr_frstname." ".$mstr_lstname; 
require_once '/nas/wp/www/cluster-2265/taichi/wp-content/themes/alamindit/mailchimp-mandrill-api-php-e1eda7352f24/src/Mandrill.php';

$mandrill = new Mandrill('ULv1lw8ehJFIQZEiLoYAkg');
$message = array(
    'subject' => 'Digital Certificate',
    'from_email' => 'usman@chimpchamp.com',
    'to' => array(array('email' => $user_id[0]->user_email, 'name' => 'Marc')),
    );
$w_message = array(
    'subject' => 'Congratulation from Dr Paul Lam',
    'from_email' => 'usman@chimpchamp.com',
    'to' => array(array('email' => $user_id[0]->user_email, 'name' => 'Marc')),
    );
...
$template_content = array(
    ...
    array(
        'name' => 'held',
        'content' => 'Held on:   '.$event_start_date),
    array(
        'name' => 'location',
    ...
    array(
        'name' => 'mt',
        'content' => $fullname_mstr));
$response = $mandrill->messages->sendTemplate($template_name, $template_content, $message);
$mandrill->messages->sendTemplate('welcome email', null, $w_message);			

// certification letters mandrill --- END
...
$totalcert = $cur_user_certs[0]->certs.",".$new_usr_certs[0]->meta_value;
$meta_key = 'certification';

$havemeta = get_user_meta($us_id,$meta_key,true); //stores the value of logged in user's meta data for 'test'.
    if ($havemeta){
        update_user_meta($us_id, $meta_key, $totalcert);
    }
   else
{add_user_meta($us_id, $meta_key, $totalcert);
}


}
add_post_meta( $order_id, 'certify', 'true' );
////////////////// mailchimp subcribe and segmentation ----start

echo $county = get_user_meta($us_id, 'country', true);
	echo $firstname = get_user_meta($us_id, 'first_name', true);
	echo $lastname = get_user_meta($us_id, 'last_name', true);
	$queryy = "SELECT seg_id FROM `wp_subscribe_segments` WHERE `seg_name` = '$county'";
	$obj = $wpdb->get_results($queryy, OBJECT);
	echo $obj[0]->seg_id;
...
$my_email = $user_info->user_email;	
$merge_vars = array('FNAME'=>$firstname, 'LNAME'=>$lastname, 'EMAIL'=>$user_info->user_email 
                    );
$retval = $api->listSubscribe( "5da4bab97a", $my_email, $merge_vars, 'html', false, true, true );
$batch[] = $user_info->user_email;
$retval = $api->listStaticSegmentMembersAdd("5da4bab97a", $obj[0]->seg_id , $batch);
if ($api->errorCode){
...
////////////////// mailchimp subcribe and segmentation ----end

wp_safe_redirect( wp_get_referer() );
}
add_action('wp_ajax_woocommerce-mark-order-certified', 'woocommerce_mark_order_certified');
