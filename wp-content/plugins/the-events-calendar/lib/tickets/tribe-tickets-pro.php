...
class TribeEventsTicketsPro {
	...
	public function attendees_row_action( $actions ) {
		...
		if ( $post->post_type == TribeEvents::POSTTYPE ) {
			...
			$actions['report'] = sprintf( '<a title="%s"  onclick="%s"  >%s</a>', __( 'See who purchased tickets to this event', 'tribe-events-calendar' ),( "javascript:void window.open('http://taichi.wpengine.com/wp-content/plugins/the-events-calendar/admin-views/generate_summery.php?post={$post->ID}','1403577113820','width=700,height=500,toolbar=0,menubar=0,location=0,status=1,scrollbars=1,resizable=1,left=0,top=0');return false;" ), __( 'Summary Report', 'tribe-events-calendar?' ) );
		}
