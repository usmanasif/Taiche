...
if(count($xyz_em_campDetails)==0){
	...
	if($xyz_em_campDetails->join_after_campaign_started == 1){
		...
		$xyz_em_mappingDetails = $wpdb->get_results( 'SELECT ea_id,id FROM '.$wpdb->prefix.'xyz_em_address_list_mapping WHERE el_id="'.$xyz_em_campDetails->list_id.'" AND status="1"
					AND id>"'.$xyz_em_campDetails->last_send_mapping_id.'" AND create_time<"'.$xyz_em_campDetails->start_time.'" ORDER BY id LIMIT 0,'.$xyz_em_campDetails->batch_size ) ;
	
	}
	
	$time = time();
	$day 	= xyz_local_date_time('d', $time);
	$month 	= xyz_local_date_time('m', $time);
	$year 	= xyz_local_date_time('Y', $time);
	$hour 	= xyz_local_date_time('H', $time);
