	...
	if (($_POST['xyz_em_campName']!= "")
		...
		if($xyz_em_startTime != ""){
			...
			if(($xyz_em_hour >= 0) && ($xyz_em_minute >=0)){
				...
				$xyz_em_currentDateTime = gmmktime($xyz_em_hour,$xyz_em_minute,$xyz_em_second,$month,$day,$year);

			}else{
				$xyz_em_currentDateTime = gmmktime(0,0,0,$month,$day,$year);
			}
		...
		if($endTimeCondition == 2){
			if($xyz_em_endTime != ""){
				...
				if(($xyz_em_hour_end >= 0) || ($xyz_em_minute_end >=0)){
					...
					$xyz_em_endDateTime = gmmktime($xyz_em_hour_end,$xyz_em_minute_end,$xyz_em_second_end,$month_end,$day_end,$year_end);
			
				}else{
					$xyz_em_endDateTime = gmmktime(0,0,0,$month_end,$day_end,$year_end);
				}
		...
		<table class="widefat" style="width:98%;">
			...
			<tr>
				...
				<td><input readonly="readonly" name="xyz_em_startTime"
					...
					value="<?php if(isset($_POST['xyz_em_startTime']) && $_POST['xyz_em_startTime'] != ""){echo $_POST['xyz_em_startTime'];}else{ echo xyz_local_date_time("m/d/Y",$details->start_time);}?>" />
					 <select
				...
				<td id="alignText">
					<div>
						...
						<div id="triggerEndTimeConditionDate"  >
							...
							value="<?php if(isset($_POST['xyz_em_endTime']) && $_POST['xyz_em_endTime'] != ""){echo $_POST['xyz_em_endTime'];}else{ echo xyz_local_date_time("m/d/Y",$details->end_time);}?>" />	
							</div>
							<div >
