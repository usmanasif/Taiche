...
function em_install(){
	...
	$tblcolums = mysql_query("SHOW COLUMNS FROM  ".$wpdb->prefix."xyz_em_email_campaign");
	while ($row = mysql_fetch_array($tblcolums)){
		if( $row['Field']=="join_after_campaign_started"){
			$group_flag=1;
		}
	}
	if($group_flag==0){
		mysql_query("ALTER TABLE ".$wpdb->prefix."xyz_em_email_campaign ADD (`join_after_campaign_started` int(1) NOT NULL DEFAULT '1')");
		//echo mysql_error();
