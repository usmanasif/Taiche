...
function get_remaining_count($listId,$lastSentMappingId,$joinAfterCampaignStarted,$campStartTime){
	...
	if($joinAfterCampaignStarted == 1){
		$remainingCount = $wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'xyz_em_address_list_mapping WHERE el_id="'.$listId.'" AND status="1" AND id>"'.$lastSentMappingId.'"' ) ;
	}elseif($joinAfterCampaignStarted == 2){
		$remainingCount = $wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'xyz_em_address_list_mapping WHERE el_id="'.$listId.'" AND status="1" AND id>"'.$lastSentMappingId.'" AND create_time<"'.$campStartTime.'"') ;
	}
