...
<h1>Master Trainers' Workshop Calendar</h1>
<p>To find a workshop, choose your country.</p>
<p><a href="http://taichi.wpengine.com/dr-lams-workshop-calendar/">Click here</a> to Dr Lam's authorised master trainers' workshop calendar.</p>
Search by country:
...
if($post->post_author == 1 || tribe_get_start_date( $post->ID, false, 'Ymd' ) < date("Ymd") || 'yes' == get_post_meta( $post->ID , '_EventHideFromUpcoming', true ) )
continue;
...
<a href="<?php the_permalink(); ?>"><u><?php echo tribe_get_start_date( $post->ID, false, 'M j-' ); echo tribe_get_end_date( $post->ID, false, 'M j' );
 ?>.</u> </a><?php the_title(); ?>,<?php echo tribe_get_city($post->ID); ?>,<?php echo tribe_get_stateprovince($post->ID) ?> <?php echo tribe_get_country($post->ID); ?>
