...
<div class="main_white_heading" style="margin: 30px 0px 10px; padding: 0px 0px 5px; border-width: 0px 0px 1px; border-bottom-style: dotted; border-bottom-color: #939393; outline: 0px; font-size: 18px; font-family: Arial, Helvetica, sans-serif; color: #133e1c; font-weight: bold; line-height: 17px; background-color: #ffffff;">CERTIFIED INSTRUCTORS</div>
&nbsp;
<h2 style="margin: 0px 0px 10px; padding: 0px 0px 5px; border-width: 0px 0px 1px; border-bottom-style: dotted; border-bottom-color: #939393; outline: 0px; font-size: 17px; color: #133e1c; font-family: Arial, Helvetica, sans-serif; line-height: 17px; background-color: #ffffff;">To find certified instructors, choose your country.</h2>
...
WHERE group_id = 4 and wp_Instructors_Certification.certification_expiry_date > curdate() and meta_key = 'country' and meta_value = '{$_GET['country']}'
...
WHERE group_id = 4 and wp_Instructors_Certification.certification_expiry_date > curdate()
    		   ";
}

 $count_display = $wpdb->get_var($count_q);
...
WHERE group_id = 4 and wp_Instructors_Certification.certification_expiry_date > curdate() and meta_key = 'country' and meta_value = '{$_GET['country']}'  limit 50
...
WHERE group_id = 4 and wp_Instructors_Certification.certification_expiry_date > curdate()  limit 50

    		   ";

}
?>
<table  id = "table_id" style = "width: 643px;" >
<thead>
<tr>
<th>Certified Instructors</th><th>State</th><th>Suburb</th><th>Qualifications</th><th hidden>country</th>
</tr>
</thead>
<tbody>
<?php
...
{	if($cert_obj['name'] == '')
  ...
  $cert_aid = $wpdb->get_var( "SELECT meta_value FROM `wp_postmeta` where post_id = $cert_id and meta_key = '_thumbnail_id'");							 $val = $wpdb->get_var( "SELECT guid  FROM wp_posts where ID =$guid_id ");
							...
							}?></td><td class = "country" id = "country" name = "country" hidden> <?php echo $cntry ?> </td>
...
<input id = "seemless" value ="<?php echo $index ?>" hidden type="text" name="seemless">
...
$.fn.dataTableExt.oApi.fnStandingRedraw = function(oSettings) {
 ...
 $("#table_id").dataTable({
	...
	"fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
      	//alert(nRow);
	//alert(var g = this.fnGetPosition(nRow));
	
	
	
	var input = aData[0]; 
	var extr = input.match(/\>(.*)\</).pop();
	extr = extr.replace(',', '');	
	$(nRow).attr("id",extr);
	$(nRow).addClass('enteries');
	
	//if(jQuery("#"+extr) != "")
	//{alert(jQuery("#"+extr));}	
	//this.fnDeleteRow(this.fnGetPosition(jQuery('#'+extr)));
	//jQuery("#table_id tbody tr#"+extr).remove();
	//alert(jQuery("#table_id tbody tr"));
	...
	"fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
       ...
       return 'Displaying ' + "10" + ' of ' +' <?php echo $count_display; ?>' + ' records --- '+'Page ' + currentPage ;
    }
	});
      var oTable;
...
setInterval(function(){data_load()},500);
oTable.fnSort( [ [2,'desc'],[3,'desc']] );
var offset=50;
var verification_array_offset =  new Array();
var index = 0;
	function data_load()
	{	
		...
		$.ajax({
    		type: "POST",
    		url: "/wp-content/themes/alamindit/main.php",
    		//dataType: "json", // Set the data type so jQuery can parse it for you
		data:{
			offset: offset,
			country: $('#msds-select').val()
		},    		
		success: function (data) {
		//alert(offset);
		
		//alert(verification_array_offset);
       		offset=offset+5;

		var abs = JSON.parse(data)
		if(abs.aaData.length < 1)
		{	
		$('#table_id_last').show();
		clearInterval(l);
		}
		...
		{
			if(abs.aaData[i][1] == undefined)
			continue;
			var input = abs.aaData[i][1]; 
			var extr = input.match(/\>(.*)\</).pop();
			verification_array_offset.push(extr);
			//alert(verification_array_offset[3]);
			//alert(verification_array_offset.indexOf(offset));
			if (verification_array_offset.indexOf(extr) == verification_array_offset.lastIndexOf(extr)) 
			{
			oTable.fnAddData([abs.aaData[i][1],abs.aaData[i][2],abs.aaData[i][3],abs.aaData[i][5],abs.aaData[i][4]], false);
...
document.getElementById("msds-select").selectedIndex = document.getElementById('seemless').value;
//oTable.fnFilter( $('#msds-select').val() );
$('#msds-select').change( function() {
            oTable.fnFilter( $(this).val() );
		//document.getElementById('seemless').value = document.getElementById("msds-select").selectedIndex;
window.location.assign("http://taichi.wpengine.com/certified-instructors/?country="+$('#msds-select').val()+"&index="+document.getElementById("msds-select").selectedIndex);       
});
